...
int __nbins__[] = { // this should always be the effective length of mybinsdef_ array minus 1 (last number there is for overflow)
  16,21,8,14,22,5,3,9,13,14,14,13,5,3,5,5,5,5,5,5
};
...
float mybinsdef_dR_gg_closestjet[n_bins+1]={0,1.0,20,20.01};
...
const float pass_veto_closejets_dRcut = 0.0; // turned off
const float pass_selection_closejets_dRcut = 1.0; // jets to be counted in the analysis
const float dR_identify_jet_with_photon = 0.5; // identification jet/photon
...
float DeltaR(float eta1, float phi1, float eta2, float phi2){
  return sqrt(pow(eta1-eta2,2)+pow(AbsDeltaPhi(phi1,phi2),2));
};
